...
describe("health route", function() {
  ...
  it("should send two responses", function(done) {
    ...
    onMessage(function(data) {
      ...
      if (messages.length === 2) {
        expect(messages).toEqual([{ health: "GOOD", count: 0 }, { health: "WARNING", count: 1 }]);
        done();
      }
    });
  });
  ...
  it("should send a change in count", function(done) {
    ...
    onMessage(function(data) {
      ...
      if (messages.length === 2) {
        expect(messages).toEqual([{ health: "WARNING", count: 2 }, { health: "WARNING", count: 1 }]);
        done();
      }
    });
  });
